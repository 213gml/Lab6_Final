...
public class RootLayoutController implements Initializable {
	...
	public void BuildMenus()
	{
		...
		mnuBet.setText("Pick Betting Rules");
		mb.getMenus().add(1,mnuBet);
		
		ToggleGroup tglGrpBet = new ToggleGroup();
		
		RadioMenuItem noLimit = new RadioMenuItem();
		noLimit.setSelected(true);
		
		RadioMenuItem potLimit = new RadioMenuItem();
		
		noLimit.setToggleGroup(tglGrpBet);
		potLimit.setToggleGroup(tglGrpBet);
	...
	public String select(){
		...
		for (Menu m: mb.getMenus())
		{
			if (m.getText() == "Pick Betting Rules")
			{
				for (MenuItem mi: m.getItems())
				{
					if (mi.getClass().equals(RadioMenuItem.class))
